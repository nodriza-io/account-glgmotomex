...
function render(sim) {
  ...
  // Modelo detallado: seguro de daños ANUAL renovable, IVA sobre los intereses y
  // comisión de contado sobre el total financiado. El Site 1 lo marca con
  // `montoFinanciado`; aquí solo cambian los rótulos y el orden del desglose.
  // Las cotizaciones viejas no lo traen y se siguen mostrando como se guardaron.
  const esDetallado = sim.montoFinanciado != null;
  const renov = esDetallado ? (sim.primasSeguro || 1) - 1 : 0;
  ...
  if (financiar != null) rows.push(row(esDetallado ? 'Financiamiento del vehículo' : 'Monto a financiar', money(financiar)));
  if (esDetallado) {
  ...
  $('view').innerHTML = `
    ...
    <div class="rv-hero${notes.length ? ' rv-hero-2' : ''}">
      <div class="rv-hero-main">
        <div class="rv-hero-label">${esDetallado ? 'Pago mensual con IVA' : 'Cuota mensual'}</div>
        <div class="rv-cuota" style="color:${t.color}">${money(sim.cuota)}</div>
        <div class="rv-hero-sub">Tasa ${pct(sim.tasa)} anual${esDetallado ? ' sin IVA' : ''} · ${escHtml(String(sim.plazo))} meses</div>
